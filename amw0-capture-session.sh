...
usage() {
  cat <<'EOF'
Usage:
  sudo ./amw0-capture-session.sh [--duration SEC] [--interval SEC] [--all]

Examples:
  sudo ./amw0-capture-session.sh
  sudo ./amw0-capture-session.sh --duration 30
  sudo ./amw0-capture-session.sh --duration 20 --interval 0.1
  sudo ./amw0-capture-session.sh --all
