...
.EXAMPLE
  powershell -ExecutionPolicy Bypass -File .\scripts\windows\windows-cc-capture.ps1

.EXAMPLE
  powershell -ExecutionPolicy Bypass -File .\scripts\windows\windows-cc-capture.ps1 -DurationSec 90 -IntervalMs 500

.EXAMPLE
  powershell -ExecutionPolicy Bypass -File .\scripts\windows\windows-cc-capture.ps1 -SnapshotOnly
#>
...
$WindowsResearchRoot = Join-Path $RepoRoot "windows-control-center"

if ([string]::IsNullOrWhiteSpace($OutDir)) {
    $stamp = Get-Date -Format "yyyyMMdd-HHmmss"
    $OutDir = Join-Path -Path $WindowsResearchRoot -ChildPath ("cc-capture-" + $stamp)
}
