...
usage() {
  cat <<'EOF'
Usage:
  sudo ./scripts/amw0/amw0-probe.sh
    Read-only dump of \_SB.AMW0.WQAA/WQAB/WQAC.

  sudo ./scripts/amw0/amw0-probe.sh call '<acpi_call expression>'
    Execute a custom acpi_call expression once.

Examples:
  sudo ./scripts/amw0/amw0-probe.sh
  sudo ./scripts/amw0/amw0-probe.sh call '\_SB.AMW0.WQAC 0x0'
  sudo ./scripts/amw0/amw0-probe.sh call '\_SB.AMW0.WMBA 0x0 0x1 0x0'
EOF
}
