...
This repository is a research workspace, not a packaged application. Top-level Markdown files such as `README.md`, `CURRENT-FINDINGS.md`, `AMW0-analysis.md`, and `windows-control-center-analysis.md` hold the current reverse-engineering narrative. Shell helpers are organized under `scripts/amw0/`, `scripts/ec/`, and `scripts/acpi/`; Windows capture helpers live under `scripts/windows/`. `windows-control-center/` stores reverse reports and string notes; vendor binaries, extracted package trees, and capture outputs should remain local and untracked. `legacy-nbfc-configs/` is historical reference only.

## Build, Test, and Development Commands
There is no Makefile or CI pipeline; development is script-driven.

- `bash -n scripts/amw0/amw0-wmbc-pack.sh scripts/amw0/amw0-wmbc-trace.sh`: syntax-check Bash helpers before commit.
- `sudo ./scripts/amw0/amw0-wmbc-pack.sh send 0x0 0x0001 0x49 0x00 0x1E 0x00`: issue one AMW0 `WMBC` send on Linux.
- `sudo ./scripts/amw0/amw0-wmbc-trace.sh wkbc 0x0000:0x49:0x00:0x1E:0x00`: capture EC-side timing and transient bytes.
- `powershell -ExecutionPolicy Bypass -File .\scripts\windows\windows-cc-capture.ps1`: collect Windows Control Center runtime evidence.
...
Do not commit ignored runtime outputs such as `amw0-logs/`, `ec-probe-logs/`, `acpi-scan/`, extracted MSIX trees, `windows-control-center/*.exe`, `windows-control-center/cc-capture-*`, `windows-control-center/cc-deep-capture-*`, or `WQBA.bin`. Avoid destructive probes; prefer read/trace helpers first and document any write-path experiment clearly.
